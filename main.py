...
def run_experiment_live(weights, parameters):
    """
    Runs a live decoding experiment with continuous input and 
    ...
    record_thread.daemon = True  # Allow the main thread to exit
    record_thread.start()
    ...
    try:
        while True:
            ...
            # Get new input images (example: random noise)
            num_pixels = 784  # Adjust as needed
            num_images = 1  # Number of images to generate
            new_images = np.random.rand(num_pixels, num_images) 
        ...
        input_queue.put(None)  # Sentinel value to stop the thread
